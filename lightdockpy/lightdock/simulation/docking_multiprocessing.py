...
def run_simulation(parser):
    ...
    try:
        ...
        if args.use_anm:
            ...
            np.savetxt("%s%s.txt" % (DEFAULT_REC_NM_FILE, NUMPY_FILE_SAVE_EXTENSION),receptor.n_modes.flatten())
            try:
                ...
                ligand.n_modes = None
            np.savetxt("%s%s.txt" % (DEFAULT_LIG_NM_FILE, NUMPY_FILE_SAVE_EXTENSION),ligand.n_modes.flatten())
